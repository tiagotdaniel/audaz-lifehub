...
export default function Hoje() {
  ...
  const TaskRow = ({ task }: { task: NonNullable<typeof allTasks>[0] }) => (
    ...
    >
      ...
      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        {task.status === "pending" && (
          <button onClick={(e) => handleExecute(task.id, e)} className="p-2 rounded-lg text-[#C9A84C] hover:bg-[#C9A84C] hover:text-[#0D1B2A] transition-colors" title="Executar">
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-serif font-bold text-[#F0EBE3] mb-1">Hoje</h1>
        <p className="text-[#A89880] text-sm">{format(new Date(), "EEEE, d 'de' MMMM yyyy", { locale: ptBR })}</p>
      </div>
      ...
      <div>
        <h2 className="text-xs font-semibold text-[#A89880] uppercase tracking-widest mb-3 flex items-center gap-2">
